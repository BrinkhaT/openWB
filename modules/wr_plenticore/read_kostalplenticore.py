...
# die von der PV erzeugt wurde, einschließlich Speicherladung), berechnet openWB selbst.
#
...
class plenticore(modbus):
    ...
    def __init__(self, WRIP, Battery):
        ...
        # Plenticore als Modbus Client einrichten
        try:
            modbus.__init__(self, WRIP)
        except:
            # kein Zugriff auf WR1, also Abbruch und mit Null initialisierte Variablen in die Ramdisk
            log.debug('Wechselrichter IP :' + self._IP)
            log.debug('Fehler beim Initialisieren des Modbus-Client WR1: ' +
                      str(sys.exc_info()[0]))
    ...
    def ReadBattery(self):
        ...
        try:
            ...
            log.debug('Fehler beim Lesen der Modbus-Register Battery:' +
                      str(self._IP) + '(falsche IP?)' + str(sys.exc_info()[0]))
            ...
            # zur weiter Berechnung im Fall mit Batterie
                ...
                if self.attr_WR.P_DC_in_total < 0:
                    ...
                    if self.attr_Bat.P_charge_discharge > 0:
                        ...
                        self.attr_WR.P_PV_AC_total = int(
                            (self.attr_WR.P_DC_in_total / float(Total_DC_power1)) * self.attr_WR.P_Generation_actual)
    ...
    def ReadKSEM300(self):
        try:
            ...
            # kein Zugriff auf WR1, also Abbruch und mit 0 initialisierte Variablen in die Ramdisk
            log.debug("Fehler beim Lesen der Modbus-Register KSEM300:" + str(self.IP)
                      + "(falsche WR-IP?)" + str(sys.exc_info()[0]))
    ...
    def BatteryMgt(self):
        ...
        try:
            ...
            # kein Zugriff auf WR1, also Abbruch und mit 0 initialisierte Variablen in die Ramdisk
            log.debug("Fehler beim Lesen der Modbus-Register BatteryMgt.:" +
                      str(self._IP) + "(falsche IP?)" + str(sys.exc_info()[0]))


def update(WR1IP: str, WR2IP: str, Battery: int, WR3IP: str):
    WR4IP = "none"
    WR5IP = "none"
    ips = ipparser(WR3IP)
    ...
    log.debug("Wechselrichter Kostal Plenticore Config - WR1:" + str(WR1IP) + " -WR2:" + str(WR2IP) +
    ...
    if WR3IP != "none":
        WR3 = plenticore(WR3IP, 0)

    # am WR2 darf keine Batterie sein, deswegen hier vereinfacht PV-Leistung = AC-Leistung des WR
    if WR4IP != "none":
        WR4 = plenticore(WR4IP, 0)

    if WR5IP != "none":
        WR5 = plenticore(WR5IP, 0)
    ...
    if WR4 is not None:
        ...
        log.debug("WR4 Leistung = " + str(WR4.attr_WR.P_PV_AC_total) +
                  "PV_total = " + str(PV_power_total))
        # Summen der Erträge bestimmen
    ...
    if Battery == 1:
        # Speicherladung muss durch Wandlungsverluste und internen Verbrauch korrigiert werden
        ...
        if WR1.attr_Bat.P_charge_discharge >= 0:
            P_Charge_corrected = WR1.attr_Bat.P_charge_discharge - \
                (WR1.attr_Bat.P_charge_discharge - WR1.attr_WR.P_Home_Cons_Bat)
    ...
    # die Variablen dürfen nicht der Nomenklatur openWB entsprechen
    # Bsp. pv1watt oder pv2watt, da diese für PV Module 1 und 2 reserviert sind
